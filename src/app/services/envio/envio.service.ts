...
export class EnvioService {
  ...
  guardar(envio: EnvioDto, cliente: number, documentacion: number, proveedor: string, tipoPaquete: string, usuario: string) {
    // console.log(envio);
    return this.http.post<EnvioDto>(`${URL}${ENVIOS}/${cliente}/${documentacion}/${proveedor}/${tipoPaquete}/${usuario}`, envio);
  }
  ...
  actualizarEstado(estadoEnvio: string, envio: EnvioDto) {
    return this.http.post<EnvioDto>(`${URL}${ENVIOS}${ACT_ENV}${estadoEnvio}`, envio);
  }

  actualizarEstadoPago(estadoPago: string, envio: EnvioDto) {
    // console.log('Desde servicio Envio: ', envio);
    return this.http.post<EnvioDto>(`${URL}${ENVIOS}${ACT_PAGO}${estadoPago}`, envio);
  }

  buscarEnvioPorGuia(guia: string) {
    return this.http.get<EnvioMostrar>(`${URL}${ENVIOS}/${BUSCAR_GUIA}${guia}`);
  }

  buscarEnvioPorFiltros(params: string) {
    return this.http.get<EnvioMostrar[]>(`${URL}${ENVIOS}${BUSQUEDA_FILTROS}${params}`);
  }
  ...
  contarEnvioProveedorPeriodo(proveedor: string, fechaInicial: string, fechaFinal: string) {
    ...
    return this.http.get<number>(`${URL}${ENVIOS}${CONTAR_PROVEEDOR_PERIODO}${proveedor}/${fechaInicial}/${fechaFinal}`);
  }
  ...
  contarEnvioFacturaPeriodo(factura: string, fechaInicial: string, fechaFinal: string) {
    ...
    return this.http.get<number>(`${URL}${ENVIOS}${CONTAR_FACTURA_PERIODO}${factura}/${fechaInicial}/${fechaFinal}`);
  }
  ...
  contarEnvioProveedorEstadoEnvioPeriodo(proveedor: string, estadoEnvio: string, fechaInicial: string, fechaFinal: string) {
    ...
    return this.http.get<number>(`${URL}${ENVIOS}${CONTAR_PROVEEDOR_ESTADOENVIO_PERIODO}${proveedor}/${ENVIO}/${estadoEnvio}/${PERIODO}/${fechaInicial}/${fechaFinal}`);
  }

  contarEnvioClientePeriodo(cliente: string, fechaInicial: string, fechaFinal) {
    return this.http.get<number>(`${URL}${ENVIOS}${CONTAR_CLIENTE}${cliente}/${PERIODO}/${fechaInicial}/${fechaFinal}`);
  }
  ...
  contarEnvioClienteEstadoEnvioPeriodo(cliente: string, estadoEnvio: string, fechaInicial: string, fechaFinal: string) {
    ...
    return this.http.get<number>(`${URL}${ENVIOS}${CONTAR_CLIENTE}${cliente}/edoEnv/${estadoEnvio}/${PERIODO}/${fechaInicial}/${fechaFinal}`);
  }
  ...
  conteoEnvioClienteFacturaPeriodo(cliente: string, estadoPago: string, fechaInicial: string, fechaFinal: string) {
    ...
    return this.http.get<number>(`${URL}${ENVIOS}${CONTAR_CLIENTE}${cliente}/edoPgo/${estadoPago}/${PERIODO}/${fechaInicial}/${fechaFinal}`);
  }

  conteoEnvioDestinoPeriodo(fechainicial: string, fechaFinal: string) {
    return this.http.get<typeof Map>(`${URL}${ENVIOS}${LISTAR_DESTINOS}${fechainicial}/${fechaFinal}`);
  }

  recuperarTodosLosClientes() {
    return this.http.get<any[]>(`${URL}${ENVIOS}/${CLIENTES}`);
  }
  ...
  guardarUsuarioTSMO(envio: EnvioDto, documentacion: number, proveedor: string, tipoPaquete: string, usuario: string) {
    ...
    return this.http.post<any>(`${URL}${ENVIOS}${GUARDAR_USER_TSMO}${documentacion}/${proveedor}/${tipoPaquete}/${usuario}`, envio);
  ...
  guardarClienteTSMO(envio: EnvioDto, documentacion: number, proveedor: string, tipoPaquete: string, usuario: string) {
    ...
    return this.http.post<any>(`${URL}${ENVIOS}${GUARDAR_CLIENTE_TSMO}${usuario}/${documentacion}/${proveedor}/${tipoPaquete}`, envio);
  }

  actualizarGuiaProveedor(envio: EnvioDto) {
    return this.http.post<any>(`${URL}${ENVIOS}${GUIA_PROVEEDOR}`, envio);
  }

  actualizarEstadoPagoTSMO(envio: number, estadoPago: string) {
    return this.http.get<number>(`${URL}${ENVIOS}${ACTUALIZAR_EDO_PAGO}${envio}/${estadoPago}`);
  }

  exportarListaExcel(params: string) {
    console.log(`${URL}${ENVIOS}${EXPORTAR_EXCEL}${params}`);
    window.open(`${URL}${ENVIOS}${EXPORTAR_EXCEL}${params}`, '_blank');
    // return this.http.post(`${URL}${ENVIOS}${EXPORTAR_EXCEL}`, envios);
  }

  imprimirGuia(guia: string) {
    return this.http.get(`${URL}${ENVIOS}${IMPRIMIR_GUIA}${guia}`, {responseType: 'arraybuffer'});
  }
